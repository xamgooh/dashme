...
export async function GET() {
  ...
  const avgCtr = daily.length ? daily.reduce((s, d) => s + d.ctr, 0) / daily.length : 0
  const avgPos = daily.length ? daily.reduce((s, d) => s + d.position, 0) / daily.length : 0

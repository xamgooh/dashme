...
export async function GET() {
  const metrics = await db.siteMetric.findMany({ orderBy: { date: 'asc' } })
  ...
  for (const m of metrics) {
    ...
    byDate[d].clicks += m.clicks; byDate[d].impressions += m.impressions
    byDate[d].ctr += m.ctr; byDate[d].position += m.position; byDate[d].count++
  }
  const daily = Object.entries(byDate).sort(([a],[b]) => a.localeCompare(b)).map(([date, v]: [string, B]) => ({
    date, clicks: v.clicks, impressions: v.impressions,
    ctr: Number((v.ctr / v.count).toFixed(1)), position: Number((v.position / v.count).toFixed(1)),
  }))
  ...
  return Response.json({ totals: { clicks: tc, impressions: ti, ctr: Number(ac.toFixed(1)), position: Number(ap.toFixed(1)), trend: Number((prev > 0 ? ((curr-prev)/prev)*100 : 0).toFixed(1)) }, daily })
}
